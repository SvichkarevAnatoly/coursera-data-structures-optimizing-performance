...
public class AutoCompleteDictionaryTrieTester {
    ...
    @Test
    public void basicAddWordAndCheck() {
        assertTrue(emptyDict.addWord("cat"));
        assertFalse(emptyDict.addWord("cat"));

        assertTrue(emptyDict.isWord("cat"));
        assertFalse(emptyDict.isWord("cab"));
    }

    @Test
    public void simplePredictions() {
        emptyDict.addWord("cat");
        List<String> cat = emptyDict.predictCompletions("ca", 1);
        assertArrayEquals(cat.toArray(), new String[]{"cat"});

        emptyDict.addWord("cab");
        assertThat(emptyDict.predictCompletions("ca", 3))
                .containsExactlyInAnyOrder("cat", "cab");
    }
